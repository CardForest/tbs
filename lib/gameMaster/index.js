...
var playerScopeSnapshotMapper = require('./playerScope').snapshotMapper;
      ...
      playersBoundary: M.Object({
        props: {
          playersReached: M.Array({
            defaultLength: this.numOfPlayers, // TODO
            elem: M.Boolean()
          }),
...
GameMaster.prototype.start = function (numOfPlayers) {
  ...
  return {
    snapshotPerPlayer: this.snapshotPerPlayer(),
    changeSetPerPlayer: this.performGameAction(this.onStart, this.game, [])
  };
};
...
GameMaster.prototype.snapshotPerPlayer = function () {
  var numOfPlayers = this.game.players.length;
  var snapshotPerPlayer = Array(numOfPlayers);
  for (var i = 0; i < numOfPlayers; i++) {
    snapshotPerPlayer[i] = (this.game.snapshot(playerScopeSnapshotMapper(i)));
  }

  return snapshotPerPlayer;
};

GameMaster.prototype.snapshot = function (playerIdx) {
  return this.game.snapshot(playerScopeSnapshotMapper(playerIdx));
};
